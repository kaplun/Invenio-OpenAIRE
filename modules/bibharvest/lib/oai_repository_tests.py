...
from invenio.config import CFG_OAI_LOAD
...
class TestErrorCodes(unittest.TestCase):
    ...
    def test_issue_error_identify(self):
        ...
        self.assertNotEqual([], [code for (code, text) in oai_repository_server.check_argd({'verb':"IllegalVerb"}) if code == 'badVerb'])
        self.assertNotEqual([], [code for (code, text) in oai_repository_server.check_argd({'verb':"Identify",
                                                                                      'test':"test"}) if code == 'badArgument'])
        self.assertNotEqual([], [code for (code, text) in oai_repository_server.check_argd({'verb':"ListIdentifiers",
        ...
        self.assertNotEqual([], [code for (code, text) in oai_repository_server.check_argd({'verb':"ListIdentifiers"}) if code == 'badArgument'])
        self.assertNotEqual([], [code for (code, text) in oai_repository_server.check_argd({'verb':"ListIdentifiers",
        ...
        self.assertNotEqual([], [code for (code, text) in oai_repository_server.check_argd({'verb':"ListRecords",
                                                                                      'metadataPrefix':"oai_dc",
                                                                                      'set':"really_wrong_set",
                                                                                      'from':"some_random_date",
                                                                                      'until':"some_random_date"}) if code == 'badArgument'])
        self.assertNotEqual([], [code for (code, text) in oai_repository_server.check_argd({'verb':"ListRecords"}) if code == 'badArgument'])

        self.assertNotEqual([], [code for (code, text) in oai_repository_server.check_argd({'verb': 'ListRecords', 'resumptionToken': ''}) if code == 'badResumptionToken'])
