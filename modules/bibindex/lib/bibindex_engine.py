...
from invenio.spellutils import spell_check
...
def get_words_from_fulltext(url_direct_or_indirect, stemming_language=None):
    ...
    try:
        if bibdocfile_url_p(url_direct_or_indirect):
            ...
            return get_words_from_phrase(bibdoc.get_text(), stemming_language, check_spelling=True)
        else:
            ...
            best_formats = [normalize_format(format) for format in CFG_WEBSUBMIT_BEST_FORMAT_TO_EXTRACT_TEXT_FROM]
            format = guess_format_from_url(url_direct_or_indirect)
            if CFG_BIBINDEX_SPLASH_PAGES.match(url):
            ...
            for url in urls:
                ...
                os.remove(tmptext)
                tmpwords = get_words_from_phrase(text, stemming_language, check_spelling=True)
...
def get_words_from_phrase(phrase, stemming_language=None, check_spelling=False):
    ...
    """
    if check_spelling and stemming_language:
        phrase = spell_check(phrase, stemming_language)
    words = {}
    formulas = []
...
def get_pairs_from_phrase(phrase, stemming_language=None, check_spelling=False):
    """Return list of words found in PHRASE.  Note that the phrase is
       split into groups depending on the alphanumeric characters and
       punctuation characters definition present in the config file.
    """
...
def task_run_core():
    ...
    wordTables = get_word_tables(task_get_option("windex"))
    for index_id, index_tags in wordTables:
        is_fulltext_index = get_index_name_from_index_id(index_id) == 'fulltext'
        reindex_prefix = ""
        if task_get_option("reindex"):
            reindex_prefix = "tmp_"
            init_temporary_reindex_tables(index_id, reindex_prefix)
        wordTable = WordTable(index_id, index_tags, reindex_prefix + 'idxWORD%02dF', get_words_from_phrase, {'8564_u': get_words_from_fulltext},  is_fulltext_index=is_fulltext_index)
        _last_word_table = wordTable
        wordTable.report_on_table_consistency()
        try:
