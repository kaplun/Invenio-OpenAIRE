...
"""
Djvu2PS WebSubmit File Converter.
"""

import os

from invenio.config import CFG_PATH_DJVU2PS
...
def converter(input_file, output_file, working_dir,  level=2, **argd):
    """
    @param level: the desired PostScript level, either 1, 2 or 3.
    @type level: int
    """
    execute_command(CFG_PATH_DJVU2PS, '-level=%s' % level, input_file, output_file)

def check_prerequisities():
    """Return True if OpenOffice tmpdir do exists and OpenOffice can
    successfully create file there."""
    return bool(CFG_PATH_DJVU2PS)
