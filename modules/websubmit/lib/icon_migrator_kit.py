...
def fix_bibdoc_bibdoc(id_bibdoc1, id_bibdoc2, logfile):
    ...
    try:
        ...
        for a_file in the_icon.list_latest_files():
            the_bibdoc.add_file_new_format(a_file.get_full_path(), format=a_file.get_format() + ':icon')
        the_icon.delete()
        run_sql("DELETE FROM bibdoc_bibdoc WHERE id_bibdoc1=%s AND id_bibdoc2=%2", (id_bibdoc1, id_bibdoc2))
        print "OK"
        print >> logfile, "OK"
        return True
    except Exception, err:
...
def fix_marc(to_fix_marc):
    """Fix the MARCXML of all the affected records."""
    ffts = {}
    for recid in to_fix_marc:
        ffts[recid] = []
...
In order for the script to go further they need to be removed.""", style='important')
    ...
    for id_bibdoc1, id_bibdoc2 in bibdoc_bibdoc:
        to_fix_marc.append(get_recid_from_docid(id_bibdoc1))
