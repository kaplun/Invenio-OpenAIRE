...
def unoconv(input_file, output_file=None, output_format='txt', pdfopt=True, **dummy):
    ...
    try:
        ...
        execute_command('sudo', '-u', CFG_OPENOFFICE_USER, 'HOME=%s' % CFG_OPENOFFICE_TMPDIR, CFG_PATH_OPENOFFICE_PYTHON, os.path.join(CFG_PYLIBDIR, 'invenio', 'unoconv.py'), '-v', '-s', CFG_OPENOFFICE_SERVER_HOST, '-p', CFG_OPENOFFICE_SERVER_PORT, '--output', tmpfile, '-f', unoconv_format, input_file)
