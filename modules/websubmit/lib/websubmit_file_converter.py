...
def _register_unoconv():
    ...
    try:
        if not _UNOCONV_DAEMON:
            _UNOCONV_DAEMON = subprocess.Popen(['sudo', '-S', '-u', CFG_OPENOFFICE_USER, CFG_PATH_OPENOFFICE_PYTHON, os.path.join(CFG_BINDIR, 'inveniounoconv'), '-s', CFG_OPENOFFICE_SERVER_HOST, '-p', str(CFG_OPENOFFICE_SERVER_PORT), '-l'], stdin=open('/dev/null', 'r'), stdout=open('/dev/null', 'r'), stderr=open('/dev/null', 'r'))
            time.sleep(3)
    finally:
...
def _unregister_unoconv():
    ...
    try:
        if _UNOCONV_DAEMON:
            subprocess.call(['sudo', '-S', '-u', CFG_OPENOFFICE_USER, CFG_PATH_OPENOFFICE_PYTHON, os.path.join(CFG_BINDIR, 'inveniounoconv'), '-k'], stdin=open('/dev/null', 'r'), stdout=open('/dev/null', 'r'), stderr=open('/dev/null', 'r'))
            time.sleep(1)
...
def unoconv(input_file, output_file=None, output_format='txt', pdfopt=True, **dummy):
    ...
    try:
        try:
            ...
            try:
                execute_command(CFG_PATH_OPENOFFICE_PYTHON, os.path.join(CFG_BINDIR, 'inveniounoconv'), '-s', CFG_OPENOFFICE_SERVER_HOST, '-p', str(CFG_OPENOFFICE_SERVER_PORT), '--output', tmpoutputfile, '-f', unoconv_format, tmpinputfile, sudo=CFG_OPENOFFICE_USER)
            except:
                register_exception(alert_admin=True)
                raise
            ...
            try:
                ...
                execute_command(CFG_PATH_OPENOFFICE_PYTHON, os.path.join(CFG_BINDIR, 'inveniounoconv'), '-k', sudo=CFG_OPENOFFICE_USER)
    ...
    execute_command(CFG_PATH_OPENOFFICE_PYTHON, os.path.join(CFG_BINDIR, 'inveniounoconv'), '-r', tmpoutputfile, sudo=CFG_OPENOFFICE_USER)
