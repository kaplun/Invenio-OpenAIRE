...
class WebSubmitInterface(list):
    ...
    def get_html(self):
        out = '<form name="input" action="submitted" method="post"> \
               <table class="websubmitng_form"><tbody bgcolor="#E3E4FA">\n'
        for element in self:
            out += '  <tr>\n'
...
class Table(WebSubmitInterfaceElement):
    ...
    def get_value(self):
        """
        Value of the table is returned as a list comprising the values
        in each of the positions row-wise.
        """
        element_values = []
        for row in self.__elements:
            for col in row:
...
class Fieldset(WebSubmitInterfaceElement):
    ...
    def get_value(self):
        """
        Value of the fieldset is returned as a list comprising the individual values.
        """
        element_values = []
        for element in self.__elements:
